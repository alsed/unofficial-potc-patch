...
void ProcessDialogEvent()
{
	...
	switch(Dialog.CurrentNode)
	{
		...
		case "ShipRepaired":
			//check for ship repaired
			//if(checkQuestAttribute("Tut_RepairShip", "completed"))
			if (PChar.ship.hp == 500)
				{
				dialog.text = DLG_TEXT[265];
				link.l1 = DLG_TEXT[266];
				link.l1.go = "FinalStage";
				}
			else 
				{
		...
		case "percent_skip":
			...
			Log_SetStringToLog(GlobalStringConvert("QUEST_MESSAGE16"));
		...
		case "Final_Goodbye_skip_2":
			...
			link.l1.go = "Exit";
			Log_SetStringToLog(GlobalStringConvert("QUEST_MESSAGE17"));
			PlayStereoSound("interface\notebook.wav");
		break;

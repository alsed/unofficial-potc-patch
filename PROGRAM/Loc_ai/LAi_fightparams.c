...
float LAi_CalcDeadExp(aref attack, aref enemy)
{
	...
	{
		Log_SetStringToLog("+" + sti(dmg) + XI_ConvertString("add experience"));
	}
	return dmg;
}
...
void LAi_ApplyCharacterBladeDamage(aref attack, aref enemy, float attackDmg, float hitDmg, bool isBlocked)
{
	...
	if(LAi_IsDead(enemy))
	{
		...
		if (attack.id == characters[GetMainCharacterIndex()].id)
		{
			if(CheckAttribute(enemy, "equip.blade"))
			{
				if ( (enemy.id == characters[GetCharacterIndex("Malcolm Hatcher")].id) ||
				    (enemy.id == characters[GetCharacterIndex("Camilo Machado")].id) ||
					(enemy.id == characters[GetCharacterIndex("Isenbrandt Jurcksen")].id) )
				{
					GiveItem2Character(GetMainCharacter(), enemy.equip.blade);
					blade_name = "itmname_" + enemy.equip.blade;
					Log_SetStringToLog("From the corpse of " + enemy.name + " " + enemy.lastname + " you retrieve a sword called " + LanguageConvertString(idLngFile,blade_name));	
				...
				else
				{
					...
					if ( roll == 1)
					{
						GiveItem2Character(GetMainCharacter(), enemy.equip.blade);
						blade_name = "itmname_" + enemy.equip.blade;
						Log_SetStringToLog(" you retrieve a sword called " + LanguageConvertString(idLngFile,blade_name));	
						PlayStereoSound("interface/important_item.wav");
					}
				}
			}

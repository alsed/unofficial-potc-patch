	...
	switch(ControlName)
	{
		case "WMapPauseSwitch":
				if (!CheckAttribute(&characters[0], "pause"))
				{
					...
					Log_SetStringToLog(XI_ConvertString("Game Paused"));
				}
				else
				{
		...
		case "TimeScalePlus":
			...
			Log_SetStringToLog(XI_ConvertString("Time x") + stf(characters[0].timescale));
		break;
		...
		if(IsPerkIntoList("TimeSpeed"))
		{
			...
			Log_SetStringToLog(XI_ConvertString("Time x") + "1");
		}
		else
		{
			SetTimeScale(3.0);
			AddPerkToActiveList("TimeSpeed");
			Log_SetStringToLog(XI_ConvertString("Time x") + "3");
		}
		break;
	}

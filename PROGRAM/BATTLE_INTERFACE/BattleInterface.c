...
void ReloadCannons(int targetNum, ref chRef)
{
	int chargeType;
	string ammoLog;
	string ammoType;
	...
	// Check if ammo already loaded
	//if (GetCurrentCharge() == chargeType)
	if (sti(chRef.Ship.Cannons.Charge.Type) == chargeType)
	{
		Log_SetStringToLog(XI_ConvertString("Ammo already loaded"));
		PlaySound("interface/" + LanguageGetLanguage() + "/_loaded.wav");
	}
	else
	{
		Log_SetStringToLog(ammoLog);
		PlaySound("interface/" + LanguageGetLanguage() + ammoType);
...
ref procGetSailTextureData()
{
	...
	if(chrIdx>=0)
	{
		...
		case PIRATE:		nationFileName = "ships\sail_Pirates.tga";		break;
		/*
		case PIRATE:

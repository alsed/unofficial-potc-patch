...
void Fort_Login(int iIslandIndex)
{
	...
	for (int i=0; i<iNumReloads; i++)
	{
		...
		if (CheckAttribute(arLocator,"fort"))
		{
			...
			{
				case FORT_NORMAL:
					if (rCharacter.Ship.Crew.Quantity > 600) rCharacter.Ship.Crew.Quantity = 600;
				break;
				case FORT_ABORDAGE:
			...
			{ 
				...
				rCharacter.Ship.Crew.Quantity = iDeadDays * 100; 
			}
			...
			if (bFortRessurect)
			{
				...
				rCharacter.Ship.Crew.Quantity = 500 + rand(100);
			}
			...
			rCharacter.Ship.Crew.Morale = 40 + rand(40);
			SetCharacterGoods(rCharacter, GOOD_BALLS, 34000);
...
float Fort_CannonDamage()
{
	...
	float fHullDamage = stf(rBall.DamageHull) * fCannonDamageMultiply * 0.8;
	float fCrewDamage = stf(rBall.DamageCrew) * fCannonDamageMultiply * 1.5;
	...
	{ 
		...
		if (iBallCharacterIndex == GetMainCharacterIndex())
		{
			...
			if (iNumDamagedCannons%3 == 0)
			{
				Log_SetStringToLog("Cannon " + iNumDamagedCannons  + " of " + iNumAllCannons + " destroyed");
			}
		}
		if (!bImmortal)
		{

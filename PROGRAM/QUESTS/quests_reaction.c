...
void FaceMaker(aref rCharacter)
{
	switch (rCharacter.model)
	{
		...
		case "pirat10":			rCharacter.FaceId = 53; break;
		case "pirat4":			rCharacter.FaceId = 54; break;
		
		case "depp":			rCharacter.FaceId = 57; break;
		case "will":			rCharacter.FaceId = 58; break;

		case "shkiper":			rCharacter.FaceId = 66; break;

		case "soldier_fra5":	rCharacter.FaceId = 104; break;
		case "soldier_fra6":	rCharacter.FaceId = 105; break;
	}
...
void QuestComplete(string sQuestName)
{
	...
	switch(sQuestName)
	{
		...
		case "Story_BlazeWithRabelAndCounterspyReturn":
			...
			DoQuestReloadToLocation("redmond_residence", "goto", "goto1", "Story_BlazeEscapedFromOxbay.over");
			LAi_type_actor_Reset(characterFromID("Rabel Iverneau"));
			LAi_type_actor_Reset(characterFromID("Counterspy"));
		...
		case "end_game":
			...
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].name = "Jack";
			characters[GetCharacterIndex("Isenbrandt Jurcksen")].lastname = "Sparrow";
		...
		case "kill_Ogario_fight":
			...
			GiveItem2Character(characterFromID("Sidonio Ogarrio"),BLADE_LONG);
			EquipCharacterByItem(characterFromID("Sidonio Ogarrio"), BLADE_LONG);
			LAi_group_MoveCharacter(characterFromID("Sidonio Ogarrio"), "OGARRIO_BODYGUARDS");

			LAi_SetHP(characterFromID("pirate for quest muelle 01"), 80.0, 80.0);
			LAi_SetHP(characterFromID("pirate for quest muelle 02"), 80.0, 80.0);
			LAi_group_MoveCharacter(characterFromID("pirate for quest muelle 01"), "OGARRIO_BODYGUARDS");
			LAi_group_MoveCharacter(characterFromID("pirate for quest muelle 02"), "OGARRIO_BODYGUARDS");
			ChangeCharacterAddressGroup(characterFromID("pirate for quest muelle 01"), "Sidonio Ogarrio House", "reload", "reload1");
			ChangeCharacterAddressGroup(characterFromID("pirate for quest muelle 02"), "Sidonio Ogarrio House", "reload", "reload1");	
			LAi_group_SetRelation("OGARRIO_BODYGUARDS", LAI_GROUP_PLAYER, LAI_GROUP_ENEMY);
			LAi_group_FightGroups("OGARRIO_BODYGUARDS", LAI_GROUP_PLAYER, true);

			LAi_ActorAttack(pchar, characterFromID("Sidonio Ogarrio"), "");
		break;
		...
		case "leaving_oxbay_exit_no_run":
			...
			AddPassenger(Pchar, characterFromID("Rabel Iverneau"), 0);
			AddPassenger(Pchar, characterFromID("Counterspy"), 0);
			LAi_QuestDelay("Story_PlayerTriesToRunFromOxbay", 0.0);
		break;
...
//Artois Voysey
		...
		case "Story_Artois_2":
			...
			LAi_group_SetRelation("QC_SOLDIERS", LAI_GROUP_PLAYER, LAI_GROUP_NEITRAL);
		...
		case "guard_informs":
			LAi_SetActorType(pchar);
			LAi_ActorTurnToCharacter(pchar, CharacterFromID("Jacinto Arcibaldo Barreto"));
			LAi_ActorWaitDialog(pchar, CharacterFromID("Jacinto Arcibaldo Barreto"));
			LAi_ActorGoToLocator(CharacterFromID("guard"), "goto", "goto9", "", 3.0);
			LAi_QuestDelay("murderer_found", 3.0);
		break;

		case "murderer_found":
			LAi_ActorDialog(characterfromID("Jacinto Arcibaldo Barreto"), PChar, "", 2.0, 1.0);
			LAi_ActorGoToLocator(CharacterFromID("guard"), "goto", "goto1", "return_post", 3.0);
			AddQuestRecord("artois", "9");
			LAi_QuestDelay("exit_trial", 5.0);
			LAi_SetPlayerType(pchar);
			//LAi_ActorWaitDialog(characterfromID("Jacinto Arcibaldo Barreto"), characterfromID("guard"));
			//LAi_ActorDialog(characterfromID("guard"), characterfromID("Jacinto Arcibaldo Barreto"), "", 1.0, 2.0);
		break;

		case "return_post":
			LAi_ActorTurnToCharacter(characterFromID("guard"), CharacterFromID("Jacinto Arcibaldo Barreto"));
		break;
	
		case "exit_trial":
			//LAi_ActorDialog(characterfromID("Jacinto Arcibaldo Barreto"), PChar, "arcibaldo_back_to_hub_type", 1.0, 1.0);
			Pchar.quest.artois_captured.win_condition.l1 = "location";
			Pchar.quest.artois_captured.win_condition.l1.location = "Conceicao_town_exit";
			PChar.quest.artois_captured.win_condition = "artois_captured";
	...
	//Nigel Blythe
		...
		case "machado_death":
			...
			ChangeCharacterAddress(characterFromID("Camilo Machado"), "None", "");
		...
		case "fourth_mission_completed":
			...
			characters[GetCharacterIndex("Arnaud Matton")].lastname = "Drunk";
		...
		case "fourth_mission_kill_arnaud_fire_fight":
			...
			characters[GetCharacterIndex("Arnaud Matton")].headModel = "h_pirate2";
			characters[GetCharacterIndex("Arnaud Matton")].name = "Toothless";
			characters[GetCharacterIndex("Arnaud Matton")].lastname = "Drinker";
			characters[GetCharacterIndex("Arnaud Matton")].location = "Falaise_de_fleur_store";
			characters[GetCharacterIndex("Arnaud Matton")].location.group = "goto";
			characters[GetCharacterIndex("Arnaud Matton")].location.locator = "goto1";
			Lai_SetStayType(characterFromID("Arnaud Matton"));
		...
		case "fifth_mission_trap_begin":
			...
			if (!CheckQuestAttribute("smuggler_barrakuda", "died"))
			{
				...
				Log_SetStringToLog(GlobalStringConvert("QUEST_MESSAGE14"));
		...
		case "pirate_line_to_black_pearl":
			...
			// new stats for Barbossa's second encounter
			characters[GetCharacterIndex("Barbossa")].rank = "30";
			characters[GetCharacterIndex("Barbossa")].perks.list.FastReload = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.HullDamageUp = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.SailsDamageUp = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.CrewDamageUp = true;	
			characters[GetCharacterIndex("Barbossa")].perks.list.BasicBattleState = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.AdvancedBattleState = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.ShipDefenseProfessional = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.CannonProfessional = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.ShipSpeedUp = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.ShipTurnRateUp = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.SailingProfessional = true;
			characters[GetCharacterIndex("Barbossa")].perks.list.CriticalShoot = true;
		...
		case "jurcksen_death":
			...
			//Log_SetStringToLog(GlobalStringConvert("QUEST_MESSAGE15"));
